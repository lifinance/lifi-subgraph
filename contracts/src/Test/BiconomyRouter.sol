...
abstract contract Pausable {
     ...
     */
    modifier onlyPauser() {
        require(isPauser(), "Only pauser is allowed to perform this operation");
        _;
    }
...
contract ExecutorManager is Ownable {
    ...
    function getExecutorStatus(address executor) public view returns (bool status) {
...
contract LiquidityPoolManager is ReentrancyGuard, Ownable, BaseRelayRecipient, Pausable {
    ...
    // MODIFIERS
    modifier onlyExecutor() {
        require(executorManager.getExecutorStatus(_msgSender()), "You are not allowed to perform this operation");
        _;
    }
    ...
    function renounceOwnership() external view override onlyOwner {
        revert("can't renounceOwnership here"); // not possible within this smart contract
    }

    function renouncePauser() external view override onlyPauser {
    ...
    function addTokenLiquidity(address tokenAddress, uint256 amount) external tokenChecks(tokenAddress) whenNotPaused {
        require(amount != 0, "Amount cannot be 0");
        address payable sender = _msgSender();
        tokensInfo[tokenAddress].liquidityProvider[sender] = tokensInfo[tokenAddress].liquidityProvider[sender].add(
            amount
        );
    ...
    function removeTokenLiquidity(address tokenAddress, uint256 amount) external tokenChecks(tokenAddress) {
        ...
        tokensInfo[tokenAddress].liquidityProvider[sender] = tokensInfo[tokenAddress].liquidityProvider[sender].sub(
            amount
        );
    ...
    function depositErc20(
        ...
        require(
            ...
            "Deposit amount should be within allowed Cap limits"
        );
    ...
    function depositNative(address receiver, uint256 toChainId) external payable whenNotPaused {
        require(
            ...
            "Deposit amount should be within allowed Cap limit"
        );
        require(receiver != address(0), "Receiver address cannot be 0");
        require(msg.value != 0, "Amount cannot be 0");
    ...
    function sendFundsToUser(
        ...
        require(
            tokensInfo[tokenAddress].minCap <= amount && tokensInfo[tokenAddress].maxCap >= amount,
            "Withdraw amount should be within allowed Cap limits"
        );
        ...
        gasFeeAccumulatedByToken[tokenAddress] = gasFeeAccumulatedByToken[tokenAddress].add(
            totalGasUsed.mul(tokenGasPrice)
        );
